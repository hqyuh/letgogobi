...
docker exec pg-replica1 psql -U postgres -d appdb -c "SELECT pg_wal_replay_pause()" >/dev/null
...
docker exec pg-primary psql -U postgres -d appdb -c "
...
docker exec pg-replica1 psql -U postgres -d appdb -c "SELECT pg_wal_replay_resume()" >/dev/null
sleep 3
